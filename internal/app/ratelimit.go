...
//   - 每分钟：30（留 2× 余量给突发）
//   - 每小时：80（实测单出口约 85 次被 302 到 sorry/index，干净连接 82-88，n=8）
//
// 注意 hourWin 用的是滚动 1 小时窗口，但那 85 次实测是间隔 1.5s 连续打出来的，
// 几分钟就打完了。阈值到底按 1 小时窗口算、按短时速率算、还是跟时间无关只看累积，
// 三者没被区分开——挂在 1 小时窗口上是保守假设，不是实测结论。
