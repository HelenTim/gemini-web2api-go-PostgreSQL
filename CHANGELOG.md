...
- **超长对话不再吃掉你刚问的那句**。上游单请求约 2 万 token 封顶，超了**从尾部
  静默截断**且不报错；而我们按顺序拼 prompt、最新消息在末尾 —— 被截掉的正好是
  用户刚问的话。表现是模型答非所问、不调工具，看着像"变笨"或"工具支持不好"。
  现在超预算会从**最旧的历史**开始丢（系统指令、工具定义和最新一条不动），
  丢的位置插一句说明让模型知道中间断过；丢完仍超则返回 400 +
  `context_length_exceeded`，而不是给一个答非所问的 200。
  上限 `max_prompt_tokens` 默认 18000，可在面板改，0 = 关掉。
