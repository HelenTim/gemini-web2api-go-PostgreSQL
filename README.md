...
| `gemini-3.1-pro` | **只在配了 cookie 时才暴露**（面板粘贴或 `--cookie-file` 都算），见下 |
...
池空才回落到上面这个单 cookie。挂了任一路径 `gemini-3.1-pro` 就会出现在模型列表里（虽然
拿不到真 Pro，见上）。

账号的「失败次数 / 最近成功」目前**不会自动更新**——请求成败还没回写到池，那两列只有手动
改状态时才动。

## 代理池（白嫖路线核心）

**为什么需要**：实测单 IP 约 **85 次请求**后就会被重定向到
`google.com/sorry/index`。

压测样本（两轮共 15 个独立出口，每个打到被拦为止）：

| 连接质量 | 被拦时的成功次数 | 极差 |
|---|---|---|
| 代理零失败 | 82 / 84 / 84 / 86 / 87 / 88 / 88 / 88 | **6** |
| 有代理失败 | 66 / 73 / 78 / 83 / 84 / 86 / 92 | 26 |

**干净连接下这个上限极其稳定（82-88）。** 一旦链路上有代理失败，上限反而会提前
（最早 66 次就被拦）——说明那些"失败"的请求里有一部分其实已经到达 Google 并被
计入了，不是完全没到。所以别指望靠"重试失败请求"多榨产能。

对照组：10 个 IP 各打 50 次（418 次请求）**零次 Google 拒绝**，失败全在代理层。
50 次以内完全安全，80 次开始接近红线，默认 `per_ip_rph=80` 就是照这个留的余量。

**被拦之后不是全死**：实测继续打 95 次仍有 14 次成功（约 15%）。所以是概率性
拦截而非硬性封禁。恢复时长本轮没测，"6-24 小时"是沿用的旧说法。
...
- **匿名访问**：单 IP 约 **85 次请求**后被重定向到 sorry 页（干净连接实测 82-88，n=8）→ 配代理池放大产能。默认 `per_ip_rph=80` 照这个留的余量
...
- **Cookie 池只轮转不健康检查**：挑账号是纯轮转，请求成败没回写，坏号不会自动禁用，得看面板手动停
