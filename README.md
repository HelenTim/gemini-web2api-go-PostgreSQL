...
挂上 cookie 额外解锁：`gemini-3.1-pro` 及其思考链、**读图**、**更长的上下文**
（超长对话自动转成文本附件发，见下文）。
...
**cookie 会自动续期**。上游几乎每个响应都会刷新 `SIDCC` / `__Secure-1PSIDCC` /
`__Secure-3PSIDCC`，我们收下并写回账号；另外每 10 分钟往 `accounts.google.com/RotateCookies`
打一次保活（间隔由服务端指定，那个响应刷的也是同一组三项）。这跟浏览器的行为一致。

**但别指望它能让号活很久**：实测同一来源的 cookie，带续期+保活的活了 86 分钟、两者都
没有的活了 114 分钟，没测出差别（n=3，且都跟真实浏览器共用同一个会话）。导出 cookie
后原浏览器继续用同一个账号刷 Gemini，两边各自轮转 `SIDCC` 很可能互相把对方顶掉——
**要长期跑，建议给这个用途单开一个账号、导出后就别再用那个浏览器登它**。
